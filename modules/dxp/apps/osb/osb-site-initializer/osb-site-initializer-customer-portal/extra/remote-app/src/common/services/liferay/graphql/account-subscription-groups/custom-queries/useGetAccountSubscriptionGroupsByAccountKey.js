/**
 * Copyright (c) 2000-present Liferay, Inc. All rights reserved.
 *
 * The contents of this file are subject to the terms of the Liferay Enterprise
 * Subscription License ("License"). You may not use this file except in
 * compliance with the License. You can obtain a copy of the License by
 * contacting Liferay, Inc. See the License for the specific language governing
 * permissions and limitations under the License, including but not limited to
 * distribution rights of the Software.
 */

import {useGetAccountSubscriptionGroups} from '..';
import {useGetKoroneikiAccountByAccountKey} from '../../koroneiki-accounts';

export function useGetAccountSubscriptionGroupsByAccountKey() {
	const {
		data: koroneikiAccountData,
		loading: koroneikiAccountLoading,
	} = useGetKoroneikiAccountByAccountKey();

	const {
		data: accountSubscriptionGroupsData,
		loading: accountSubscriptionGroupsLoading,
	} = useGetAccountSubscriptionGroups({
		filter: `(accountKey eq '${koroneikiAccountData?.accountKey}') and (hasActivation eq true)`,
		skip: koroneikiAccountLoading,
	});

	const accountSubscriptionGroupsItems =
		accountSubscriptionGroupsData?.c?.accountSubscriptionGroups?.items;

	return {
		accountSubscriptionGroups: accountSubscriptionGroupsItems,
		koroneikiAccount: koroneikiAccountData,
		loading: {
			accountSubscriptionGroups: accountSubscriptionGroupsLoading,
			koroneikiAccount: koroneikiAccountLoading,
		},
	};
}
